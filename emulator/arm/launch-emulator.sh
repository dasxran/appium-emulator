...
log_version_info() {
  ...
  echo 'version: launch_script: 0+untagged.240.g410ef1e'
  img=$ANDROID_SDK_ROOT/system-images/android-25/google_apis
  [ -f "$img/armeabi-v7a/source.properties" ] && cat "$img/armeabi-v7a/source.properties" | sed -u 's/^/version: /g'
}
...
var_append LAUNCH_CMD -skip-adb-auth -no-boot-anim -no-audio -no-snapshot
...
var_append LAUNCH_CMD -gpu swiftshader_indirect  -metrics-collection
